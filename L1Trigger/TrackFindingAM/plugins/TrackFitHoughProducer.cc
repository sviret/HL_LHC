...
TrackFitHoughProducer::TrackFitHoughProducer( const edm::ParameterSet& iConfig )
{
  ...
  TTTrackOutputTag   = iConfig.getParameter< std::string >( "TTL1TracksName" );
  TTStubOutputTag    = iConfig.getParameter< std::string >( "TTL1StubsName" );
  TTClusOutputTag    = iConfig.getParameter< std::string >( "TTL1ClustersName" );
...
void TrackFitHoughProducer::produce( edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  if ( TTPatternHandle->size() > 0 )
  {
	...
	long hit_id = (long)layer*10000000000+(long)ladder*100000000+module*100000+segment*10000+strip;
	...
	//cout<<"Track "<<tt<<" PT="<<tracks[tt]->getCurve()<<" ETA="<<tracks[tt]->getEta0()<<" PHI="<<tracks[tt]->getPhi0()<<" ZO="<<tracks[tt]->getZ0()<<endl;
	delete tracks[tt];
      }
