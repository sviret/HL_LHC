...
void do_layers_1D_map(std::string filename, bool inhz)
{
  ...
  TH2F *cadres  = new TH2F("c_s","c_s",10*n_mod_barrel[0],-100.,100.,100,0.,1.1*maxsval);
  TH2F *cadrec  = new TH2F("c_c","c_c",10*n_mod_barrel[0],-100.,100.,100,0.,1.1*maxcval);
  TH2F *cadrer  = new TH2F("c_r","c_r",10*n_mod_barrel[0],-100.,100.,100,0.,1.1*maxrval);
  ...
  for (int i=0;i<6;++i)
  {
    TH2F *cadre_stub = new TH2F("","",10*n_mod_barrel[i],-100.,100.,100,0.,1.1*maxsval);
    TH2F *cadre_clus = new TH2F("","",10*n_mod_barrel[i],-100.,100.,100,0.,1.1*maxcval);
    TH2F *cadre_rati = new TH2F("","",10*n_mod_barrel[i],-100.,100.,100,0.,1.1*maxrval);
  ...
  for (int i=0;i<6;++i)
  {
	...
	ratec *= bit_per_s*40.; // Give the value in MHz/cm2
      ...
      stub_plots[i]->Fill(-100+bin_w*(j+0.5),rates);

      clus_plots[i]->Fill(-100+bin_w*(j+0.5),ratec);

      rat_plots[i]->Fill(-100+bin_w*(j+0.5),rater);
    }
  }
  ...
  cadres->GetXaxis()->SetTitle("Module Z position (a.u.)");
  cadres->GetYaxis()->SetTitle("Stub rate (in Stub/Module/BX)"); 

  if (inhz)  cadres->GetYaxis()->SetTitle("Stub rate (in MHz/cm^{2})"); 

  cadres->GetXaxis()->SetLabelFont(42);
  cadres->GetXaxis()->SetLabelSize(0.035);
  cadres->GetXaxis()->SetTitleSize(0.04);
  ...
  cadres->GetYaxis()->SetTitleSize(0.04);
  ...
  c2 = new TCanvas("c2","Layer map",201,77,1470,858);
  gStyle->SetOptStat(0);
  gStyle->SetOptTitle(0);
  c2->Range(-5.887851,-1.930603,70.65421,17.37543);
  c2->SetFillColor(0);
  ...
  cadrec->GetXaxis()->SetTitle("Module Z position (a.u.)");
  cadrec->GetYaxis()->SetTitle("Cluster rate (in Cluster/Module/BX)"); 
  ...
  cadrec->GetXaxis()->SetTitleSize(0.04);
  ...
  cadrec->GetYaxis()->SetTitleSize(0.04);
  ...
  for (int i=0;i<6;++i)
  {
    clus_plots[i]->SetMarkerStyle(20);
    clus_plots[i]->SetMarkerColor(color[i]);
    clus_plots[i]->SetMarkerSize(2.);
    clus_plots[i]->Draw("same");
  }

  leg = new TLegend(0.75,0.75,0.95,1.);
  char buffer[50];
  ...
  {
    sprintf (buffer, "#rho = %d cm",radii[i]);
    leg->AddEntry(clus_plots[i],buffer,"p");
  }
  ...
  T2.DrawLatex(-90., maxcval, "CMS Preliminary Simulation");  
  T2.SetTextSize(0.05);
  T2.DrawLatex(-80., 0.9*maxcval, "Barrel");
  c2->Modified();
  
  c2->Update();
  
  sprintf (buffer, "Barrel_c_rate_140_%d.eps", inhz);
  c2->Print(buffer);

  sprintf (buffer, "Barrel_c_rate_140_%d.png", inhz);
  c2->Print(buffer);
  
  c2->Update();


  c3 = new TCanvas("c3","Layer map",201,77,1470,858);
  gStyle->SetOptStat(0);
  gStyle->SetOptTitle(0);
  c3->Range(-5.887851,-1.930603,70.65421,17.37543);
  c3->SetFillColor(0);
  ...
  cadrer->GetXaxis()->SetTitle("Module Z position (a.u.)");
  cadrer->GetYaxis()->SetTitle("Cluster over stub multiplicities"); 

  cadrer->GetXaxis()->SetLabelFont(42);
  cadrer->GetXaxis()->SetLabelSize(0.035);
  cadrer->GetXaxis()->SetTitleSize(0.04);
  ...
  cadrer->GetYaxis()->SetTitleSize(0.04);
  ...
  for (int i=0;i<6;++i)
  {
    rat_plots[i]->SetMarkerStyle(20);
    rat_plots[i]->SetMarkerColor(color[i]);
    rat_plots[i]->SetMarkerSize(2.);
    rat_plots[i]->Draw("same");
  }

  leg = new TLegend(0.75,0.1,0.95,0.35);
  char buffer[50];
  ...
  {
    sprintf (buffer, "#rho = %d cm",radii[i]);
    leg->AddEntry(rat_plots[i],buffer,"p");
  }
  ...
  T3.DrawLatex(-90., 0.2*maxrval, "CMS Preliminary Simulation");  
  T3.SetTextSize(0.05);
  T3.DrawLatex(-80., 0.1*maxrval, "Barrel");
  c3->Modified();
  
  c3->Update();
  
  sprintf (buffer, "Barrel_ratio_140_%d.eps", inhz);
  c3->Print(buffer);

  sprintf (buffer, "Barrel_ratio_140_%d.png", inhz);
  c3->Print(buffer);
...
void do_disks_1D_map(std::string filename, bool inhz)
{
  ...
  //  cout << maxsval << " / " << maxcval << " / " << maxrval << endl;
  ...
  for (int i=7;i<12;++i)
  {
	...
	ratec *= bit_per_s*40.; // Give the value in MHz/cm2
      ...
      if (rates !=0) stub_plots[i-7]->Fill(radii[j-100*i],rates);
      if (ratec !=0) clus_plots[i-7]->Fill(radii[j-100*i],ratec);
      if (rater !=0) rat_plots[i-7]->Fill(radii[j-100*i],rater);
    }
  }
  ...
  cadres->GetXaxis()->SetTitle("#rho (in cm)");
  cadres->GetYaxis()->SetTitle("Stub rate (in Stub/Module/BX)"); 
  ...
  cadrec->GetXaxis()->SetTitle("#rho (in cm)");
  cadrec->GetYaxis()->SetTitle("Cluster rate (in Cluster/Module/BX)"); 
  ...
  Tl.SetTextSize(0.05);
  Tl.DrawLatex(60., 0.6*maxcval, "Endcap");	
  c2->Modified();  
  c2->Update();
  
  sprintf (buffer, "Endcap_c_rate_140_%d.eps", inhz);
  c2->Print(buffer);

  sprintf (buffer, "Endcap_c_rate_140_%d.png", inhz);
  c2->Print(buffer);
  ...
  cadrer->GetXaxis()->SetTitle("#rho (in cm)");
  cadrer->GetYaxis()->SetTitle("Cluster over stub multiplicities"); 
  cadrer->GetXaxis()->SetLabelFont(42);
  cadrer->GetXaxis()->SetLabelSize(0.035);
  cadrer->GetXaxis()->SetTitleSize(0.04);
  ...
  sprintf (buffer, "Endcap_ratio_140_%d.eps", inhz);
  c3->Print(buffer);

  sprintf (buffer, "Endcap_ratio_140_%d.png", inhz);
  c3->Print(buffer);
