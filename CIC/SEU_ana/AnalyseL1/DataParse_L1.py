...
last=0
ncycles=4
hsize = 63 # Output L1 word header
...
for line in lines:
    ...
    if line.find('Standard')!=-1:
        ...
        sL1line = "".join(L1O) # The complete L1 stream

        '''
        if verbose==1:
            outlog.write('L1 input vector '+sL1line+'\n')
        '''
        ...
        if freq==320: # For 320M the output bits are duplicated by the firmware
            for i in range(int(len(L1O)/8)):
                ...
                for j in range(8):
                    ...
                    L1word.append(L1O[8*i+j]) # Dupplication
                sL1word="".join(L1word)
    ...
    if line.find('|')!=-1:  # Parsing the error line from SEU log file
        ...
        # Delta is 20 for Louvain 2, 19 for Louvain 1
        ...
        if error_type[2:4]=='01' or error_type[2:4]=='03' or error_type[2:4]=='07':
            ...
            if freq==320:
                ...
                datword2=dfwd[1]+dfwd[3]+dfwd[5]+dfwd[7]+dfwd[9]+dfwd[11]+dfwd[13]+dfwd[15]
            else:
...
for cycle in errbycycles:
    ...
    # Here we consider the special case of a single flip not in the header

    if len(cycle)-1==1 and cycle[1][6]==1 and hdr==0:
        for i in range(len(cycle)-1):
            nflip+=cycle[1][6]
        if verbose==1:
            outlog.write(str(cycle[0])+" has a simple flip in the payload"+'\n')
    else:
