...
if [ ${1} = "PR" ]; then
    ...
    cd $TOP
    cp $CMSSW_PROJECT_SRC/src/L1Trigger/TrackFindingAM/test/batch/base/AMPR_base.py BH_dummy_${SECBK}.py 

    # Finally the script is modified according to the requests
    
    sed "s/NEVTS/$STOP/"                                   -i BH_dummy_${SECBK}.py
    sed "s/NSKIP/$START/"                                  -i BH_dummy_${SECBK}.py
    sed "s#INPUTFILENAME#file:$INPUT#"                     -i BH_dummy_${SECBK}.py
    sed "s#OUTPUTFILENAME#$OUTPUT#"                        -i BH_dummy_${SECBK}.py
    sed "s#BANKFILENAME#$BK#"                              -i BH_dummy_${SECBK}.py
    sed "s/MYGLOBALTAG/$GT/"                               -i BH_dummy_${SECBK}.py
    sed "s/THRESHOLD/$thresh/"                             -i BH_dummy_${SECBK}.py
    sed "s/PATTCONT/AML1Patternsb$SEC/"                    -i BH_dummy_${SECBK}.py

    cmsRun BH_dummy_${SECBK}.py 

    rm BH_dummy_${SECBK}.py 
...
if [ ${1} = "MERGE" ]; then
    ...
    do
      ...
      echo "cms.InputTag(\"TTPatternsFromStub\", \"AML1Patternsb"${compteur}"\")" >> temp_$FNAME
